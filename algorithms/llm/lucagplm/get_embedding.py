...
try:
    ...
    from algorithms.utils import set_seed, to_device, get_labels, get_parameter_number, seq_is_gene, \
        gene_seq_replace, clean_seq, available_gpu_id, download_trained_checkpoint_lucaone, calc_emb_filename_by_seq_id
...
def main(model_args):
    ...
    if model_args.input_file:
        ...
        if seq_is_gene(model_args.seq) and model_args.seq_type != 'gene':
            print("Error! the input seq detection is gene, but the arg: --seq_type=%s is not gene" % model_args.seq_type)
            return
